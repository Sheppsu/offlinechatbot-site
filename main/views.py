...
def set_osu_data_verified(user):
    db = get_database()
    cursor = db.cursor()
    cursor.execute(f"SELECT osu_user_id FROM osu_data WHERE user = {user.twitch_name!r}")
    if cursor.fetchone():
        cursor.execute(f"UPDATE osu_data SET osu_user_id = {user.osu_id}, osu_username = {user.osu_username!r}, verified = 1 WHERE user = {user.twitch_name!r}")
    else:
        cursor.execute(f"INSERT INTO osu_data (osu_user_id, osu_username, verified) VALUES ({user.osu_id}, {user.osu_username!r}, 1)")
    db.commit()
