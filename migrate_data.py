...
class OldDatabase:
    CONNINFO = os.getenv("OLD_PG_DB_CONN")
...
class Migrator:
    ...
    def insert_users(self):
        ...
        UserOsuData.objects.bulk_create([
            UserOsuData(id=user[2], username=user[3], global_rank=0)
            for user in self.data if user[2] is not None
        ])

        UserOsuConnection.objects.bulk_create([
            UserOsuConnection(user_id=user[0], osu_id=user[2], is_verified=False)
            for user in self.data if user[2] is not None
        ])
